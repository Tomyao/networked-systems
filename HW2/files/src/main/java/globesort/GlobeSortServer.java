...
public class GlobeSortServer {
    ...
    static class GlobeSortImpl extends GlobeSortGrpc.GlobeSortImplBase {
        ...
        @Override
        public void sortIntegers(IntArray req, final StreamObserver<Test> responseObserver) {
            Integer[] values = req.getValuesList().toArray(new Integer[req.getValuesList().size()]);
            long startTime = System.currentTimeMillis();
            Arrays.sort(values);
            long endTime = System.currentTimeMillis();
            Test.Builder responseBuilder = Test.newBuilder();
            for(Integer val : values) {
                responseBuilder.addValues(val);
            }
            long time_taken = endTime - startTime;
            responseBuilder.setTimeTaken(time_taken);
            Test response = responseBuilder.build();
            responseObserver.onNext(response);
            responseObserver.onCompleted();
        }
    }
